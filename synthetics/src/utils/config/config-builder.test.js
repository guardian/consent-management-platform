...
import { ConfigAMPArticleURl, ConfigArticleUrl, ConfigBuilder, ConfigFrontUrl, IframeDomainUrl, IframeDomainUrlSecondLayer } from "./config-builder";
...
describe("ConfigBuilder", () => {
		...
		it("should construct the config object with the correct values", () => {
			...
			expect(config).toEqual({
				...
				iframeDomainUrlSecondLayer:
					IframeDomainUrlSecondLayer.CODE,
	...
	describe("getIframeDomainUrlSecondLayer", () => {
		it("should return the correct iframe domain URL for each stage", () => {
			expect(ConfigBuilder.getIframeDomainUrlSecondLayer(STAGES.PROD)).toBe(
				IframeDomainUrlSecondLayer.PROD,
			);
			expect(ConfigBuilder.getIframeDomainUrlSecondLayer(STAGES.CODE)).toBe(
				IframeDomainUrlSecondLayer.CODE,
			);
			expect(ConfigBuilder.getIframeDomainUrlSecondLayer(STAGES.LOCAL)).toBe(
				IframeDomainUrlSecondLayer.LOCAL,
			);
			expect(ConfigBuilder.getIframeDomainUrlSecondLayer("INVALID_STAGE")).toBe(
				IframeDomainUrlSecondLayer.CODE,
			);
		});
	});
