...
export const mainCheck = async function (config: Config): Promise<void> {
	...
	await checkPages(
		config,
		`${config.frontUrl}?adtest=fixed-puppies&_sp_env=stage`,
		`${config.articleUrl}?adtest=fixed-puppies&_sp_env=stage`,
	);

	// Testing the user first visits only an article page
	await checkPages(config, `${config.articleUrl}?adtest=fixed-puppies&_sp_env=stage`, '');
