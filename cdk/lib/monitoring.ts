...
export class Monitoring extends GuStack {
	constructor(scope: App, id: string, props: GuStackProps) {
		...
		const canaryName = `${lambdaBaseName}-canary-${region}-${stage.toLowerCase()}`;
		// const bucketName = region != '' ? `com-gu-frontend-artifacts-${region}`: 'aws-frontend-artifacts';

		new synthetics.Canary(this, 'CmpMonitoringCanary', {
			...
			artifactsBucketLocation: {
				...
				prefix: `${lambdaBaseName}/${stage}/artifacts`, // TO CHANGE
			},
			...
			test: synthetics.Test.custom({
				...
				code: synthetics.Code.fromBucket(
					...
					`${lambdaBaseName}/${stage}/${lambdaBaseName}-lambda-${region}/${lambdaBaseName}-lambda.zip`, // TO CHANGE
				),
			}),
