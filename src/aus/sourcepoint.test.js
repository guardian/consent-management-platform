...
describe('Sourcepoint CCPA', () => {
	...
	it("should initialize window._sp_ccpa with the correct config if it doesn't exist", () => {
		...
		expect(window._sp_ccpa.config.siteHref).toEqual(
			'https://au.theguardian.com',
		);
		expect(window._sp_ccpa.config.targetingParams.framework).toEqual('aus');
	...
	it('injects the lib', () => {
		init();
		expect(document.getElementById('sourcepoint-aus-lib')).toBeTruthy();
	});
